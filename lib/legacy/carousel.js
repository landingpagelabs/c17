...
const SLOTS = {
  // 412 (не 407): бічні картки нахилені на 11° з perspective, тож їхній
  // ближній край ПРОЄКТУЄТЬСЯ у ~412px — плоска центральна картка при 407
  // виглядала нижчою за сусідів.
  "0": { w: 348, h: 412, x: 0, ry: 0, o: 1, zi: 50 },
...
function initCarousel() {
  ...
  const openModal = () => {
    // Кожна картка має власне відео (data-card-video з CMS); якщо його нема —
    // граємо спільне відео сцени.
    const activeCard = cards[active];
    const url = (activeCard && activeCard.dataset.cardVideo)
      ? toEmbed(activeCard.dataset.cardVideo)
      : VIDEO_URL;
    videoBox.innerHTML =
      '<iframe src="' + url + '" title="Video review" allow="autoplay; encrypted-media; fullscreen" allowfullscreen></iframe>';
  ...
  if (track) {
    ...
    track.addEventListener("pointerdown", (e) => {
      ...
      hovering = true;
      stopAuto();
      // НЕ захоплюємо pointer тут: capture ретаргетить наступний click на
      // track, і кліки по бічних картках (snap у центр) ніколи не спрацюють.
      // Захоплюємо лише коли рух реально став перетягуванням (див. нижче).
    });

    track.addEventListener("pointermove", (e) => {
      if (!dragging) return;
      const dx = e.clientX - startX;
      if (Math.abs(dx) > DRAG_THRESHOLD && !dragMoved) {
        dragMoved = true;
