...
// CAMPAIGN strapline: ротація айтемів кожні 3с ---------------------------
...
(function () {
  ...
  var QUOTE_ENDPOINT = "https://formspree.io/f/xaqrdkgy";

  // "Робочий email": відсіюємо споживчі домени
  var FREE_MAIL = /@(gmail|googlemail|yahoo|ymail|hotmail|outlook|live|msn|aol|icloud|me|mac|proton|protonmail|gmx|mail|yandex)\.[a-z.]+$/i;
  function isWorkEmail(v) {
    return EMAIL_RE.test(v) && !FREE_MAIL.test(v);
  }
  ...
  if (contacts[1]) {
    var emailErrEl = contacts[1].querySelector(".quote_error");
    var emailErrDefault = emailErrEl ? emailErrEl.textContent : "";
    checks.push({
      el: contacts[1],
      ok: function () {
        var v = contacts[1].querySelector("input").value.trim();
        // Пояснюємо ЧОМУ саме невалідно: формат чи не-робочий домен
        if (emailErrEl) {
          emailErrEl.textContent =
            EMAIL_RE.test(v) && !isWorkEmail(v)
              ? "Please use your work email address"
              : emailErrDefault;
        }
        return isWorkEmail(v);
      },
    });
  }
  ...
  // Відправка у Formspree: усі вибрані значення калькулятора + контакти.
  var sending = false;
  function sendQuote() {
    if (sending) return;
    sending = true;
    submit.classList.add("is-sending");
    var oldErr = form.querySelector(".quote_form-error");
    if (oldErr) oldErr.remove();

    var payload = {
      name: contacts[0] ? contacts[0].querySelector("input").value.trim() : "",
      email: contacts[1] ? contacts[1].querySelector("input").value.trim() : "",
      website: contacts[2] ? contacts[2].querySelector("input").value.trim() : "",
      meetings_per_month: (form.querySelector('input[name="quote-meetings"]:checked') || {}).value || "",
      monthly_budget: (form.querySelector('input[name="quote-budget"]:checked') || {}).value || "",
      customer_ltv_usd: numInput ? numInput.value.replace(/[^\d]/g, "") : "",
      close_rate_percent: range ? range.value : "",
      _subject: "New quote and ROI request from c17.ai",
    };
    ...
    fetch(QUOTE_ENDPOINT, {
      ...
      .then(function (r) {
        ...
        // Успіх: ховаємо поля, показуємо просте підтвердження
        var body = form.querySelector(".quote_form-body");
        var bottom = form.querySelector(".quote_form-bottom");
        if (body) body.style.display = "none";
        if (bottom) bottom.style.display = "none";
        var ok = document.createElement("div");
        ok.className = "quote_success";
        ok.innerHTML =
          '<p class="quote_success-title">Thanks' +
          (payload.name ? ", " + payload.name.split(" ")[0] : "") +
          '!</p><p class="quote_success-text">We’re putting your quote and ROI estimate together now — keep an eye on your inbox.</p>';
        form.appendChild(ok);
      })
      .catch(function () {
        // Гучний фейл: лід не має загубитись мовчки
        var err = document.createElement("p");
        err.className = "quote_form-error";
        err.textContent =
          "Something went wrong sending this. Please try again, or email enzo@c17.ai directly.";
        form.querySelector(".quote_form-bottom").appendChild(err);
      })
      .finally(function () {
        sending = false;
...
(function () {
  ...
  // Ротація пулу: під час glitch-мерехтіння підмінюємо чіп (лого + підпис)
  // на наступний з пулу. Свап відбувається на ~350мс анімації (0.9s), коли
  // clip-path ховає контент — виглядає як "переключення каналу".
  function advancePool(card) {
    var variants = card.querySelectorAll(".chip-variant");
    if (!variants.length) return;
    var poolSize = 0;
    variants.forEach(function (v) {
      poolSize = Math.max(poolSize, parseInt(v.dataset.pool, 10) + 1);
    });
    if (poolSize < 2) return;
    var next = ((card._poolIdx || 0) + 1) % poolSize;
    card._poolIdx = next;
    variants.forEach(function (v) {
      v.classList.toggle("is-active", parseInt(v.dataset.pool, 10) === next);
    });
  }

  // Хвиля: картки glitch-яться по черзі зі стагером. Перша хвиля (1.5с) —
  // лише мерехтіння: лідери (головні клієнти) мають прожити перші ~10с.
  var waveCount = 0;
  function glitchWave() {
    var rotate = waveCount > 0;
    waveCount++;
    cards.forEach(function (card, idx) {
      setTimeout(function () {
        ...
        if (rotate) setTimeout(function () { advancePool(card); }, 350);
      }, idx * 150);
    });
  }
